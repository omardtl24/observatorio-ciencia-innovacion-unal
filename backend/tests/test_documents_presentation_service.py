...
class TestDocumentPresentationServiceCreate:
    ...
    def test_create_document_presentation_with_all_fields(self, app):
        ...
        with app.app_context():
            doc = DocumentPresentationService.create(
                main_title="Q4 Report",
                auxiliary_title="Aux Doc",
                description="Fourth quarter financial report"
            )
            
            assert doc.id is not None
            assert doc.main_title == "Q4 Report"
            assert doc.auxiliary_title == "Aux Doc"
    ...
    def test_create_document_presentation_with_minimal_fields(self, app):
        """Test creating a document presentation with only required field (main_title)."""
        with app.app_context():
            doc = DocumentPresentationService.create(main_title="Presentation")
            
            assert doc.id is not None
            assert doc.main_title == "Presentation"
            assert doc.auxiliary_title is None
            assert doc.description is None
    
    def test_create_document_presentation_without_name_fails(self, app):
        """Test that creating without a main_title raises an error."""
        with app.app_context():
    ...
    def test_create_document_presentation_sets_created_at_timestamp(self, app):
        ...
        with app.app_context():
            ...
            doc = DocumentPresentationService.create(main_title="Timestamp Doc")
...
class TestDocumentPresentationServiceRead:
    ...
    def test_get_all_documents(self, app):
        ...
        with app.app_context():
            ...
            doc2 = DocumentPresentationService.create(main_title="Doc 2", description="Second doc")
    ...
    def test_get_all_documents_as_dict(self, app):
        ...
        with app.app_context():
            DocumentPresentationService.create(main_title="Dict Doc", description="Test description")
            
            docs_dict = DocumentPresentationService.get_all_dict()
            
            assert len(docs_dict) == 1
            assert docs_dict[0]["main_title"] == "Dict Doc"
    
    def test_get_document_by_id(self, app):
        """Test getting a document by its ID."""
        with app.app_context():
            doc = DocumentPresentationService.create(main_title="Get By ID Doc")
            
            retrieved = DocumentPresentationService.get_by_id(doc.id)
            
            assert retrieved.id == doc.id
            assert retrieved.main_title == "Get By ID Doc"
...
class TestDocumentPresentationServiceUpdate:
    ...
    def test_update_document_name(self, app):
        ...
        with app.app_context():
            doc = DocumentPresentationService.create(main_title="Old Name", description="Test")
            
            updated = DocumentPresentationService.update(doc.id, main_title="New Name")
    ...
    def test_update_document_multiple_fields(self, app):
        ...
        with app.app_context():
            doc = DocumentPresentationService.create(main_title="Original")
            
            updated = DocumentPresentationService.update(
                doc.id,
                main_title="Updated Document",
                description="Updated description"
            )
            
            assert updated.main_title == "Updated Document"
    ...
    def test_update_nonexistent_document_raises_error(self, app):
        ...
        with app.app_context():
            with pytest.raises(NotFoundError):
                DocumentPresentationService.update(9999, main_title="New")
    
    def test_update_preserves_created_at(self, app):
        """Test that updating preserves created_at timestamp."""
        with app.app_context():
            doc = DocumentPresentationService.create(main_title="Test Doc")
            original_created_at = doc.created_at
            
            DocumentPresentationService.update(doc.id, main_title="Updated Doc")
...
class TestDocumentPresentationServiceDelete:
    ...
    def test_delete_document(self, app):
        ...
        with app.app_context():
            doc = DocumentPresentationService.create(main_title="Temp Doc")
            doc_id = doc.id
...
class TestDocumentPresentationServiceIntegration:
    ...
    def test_complete_crud_cycle(self, app):
        ...
        with app.app_context():
            # Create
            doc = DocumentPresentationService.create(main_title="CRUD Doc", description="For testing")
            doc_id = doc.id
            
            # Read
            retrieved = DocumentPresentationService.get_by_id(doc_id)
            assert retrieved.main_title == "CRUD Doc"
            
            # Update
            updated = DocumentPresentationService.update(doc_id, main_title="Updated Doc")
            assert updated.main_title == "Updated Doc"
            
            # Verify update
            verified = DocumentPresentationService.get_by_id(doc_id)
            assert verified.main_title == "Updated Doc"
    ...
    def test_multiple_documents_lifecycle(self, app):
        ...
        with app.app_context():
            docs = [
                DocumentPresentationService.create(main_title=f"Doc {i}", description=f"Description {i}")
                for i in range(1, 4)
            ]
            ...
            DocumentPresentationService.create(main_title="New Doc")
            assert len(DocumentPresentationService.get_all()) == 3
