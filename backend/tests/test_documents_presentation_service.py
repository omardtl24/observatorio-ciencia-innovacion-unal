...
class TestDocumentPresentationServiceCreate:
    ...
    def test_create_document_presentation_with_minimal_fields(self, app):
        """Test creating a document presentation with only required field (name)."""
        with app.app_context():
            doc = DocumentPresentationService.create(name="Presentation")
            
            assert doc.id is not None
            assert doc.name == "Presentation"
            assert doc.description is None
    
    def test_create_document_presentation_without_name_fails(self, app):
        """Test that creating without a name raises an error."""
        with app.app_context():
    ...
    def test_create_document_presentation_sets_created_at_timestamp(self, app):
        ...
        with app.app_context():
            ...
            doc = DocumentPresentationService.create(name="Timestamp Doc")
...
class TestDocumentPresentationServiceRead:
    ...
    def test_get_all_documents(self, app):
        ...
        with app.app_context():
            doc1 = DocumentPresentationService.create(name="Doc 1")
            doc2 = DocumentPresentationService.create(name="Doc 2", description="Second doc")
    ...
    def test_get_all_documents_as_dict(self, app):
        ...
        with app.app_context():
            DocumentPresentationService.create(name="Dict Doc", description="Test description")
            
            docs_dict = DocumentPresentationService.get_all_dict()
            
            assert len(docs_dict) == 1
            assert docs_dict[0]["name"] == "Dict Doc"
    
    def test_get_document_by_id(self, app):
        """Test getting a document by its ID."""
        with app.app_context():
            doc = DocumentPresentationService.create(name="Get By ID Doc")
            
            retrieved = DocumentPresentationService.get_by_id(doc.id)
            
            assert retrieved.id == doc.id
            assert retrieved.name == "Get By ID Doc"
...
class TestDocumentPresentationServiceUpdate:
    ...
    def test_update_document_name(self, app):
        ...
        with app.app_context():
            doc = DocumentPresentationService.create(name="Old Name", description="Test")
            
            updated = DocumentPresentationService.update(doc.id, name="New Name")
    ...
    def test_update_document_multiple_fields(self, app):
        ...
        with app.app_context():
            doc = DocumentPresentationService.create(name="Original")
    ...
    def test_update_nonexistent_document_raises_error(self, app):
        ...
        with app.app_context():
            with pytest.raises(NotFoundError):
                DocumentPresentationService.update(9999, name="New")
    
    def test_update_preserves_created_at(self, app):
        """Test that updating preserves created_at timestamp."""
        with app.app_context():
            doc = DocumentPresentationService.create(name="Test Doc")
            original_created_at = doc.created_at
            
            DocumentPresentationService.update(doc.id, name="Updated Doc")
...
class TestDocumentPresentationServiceDelete:
    ...
    def test_delete_document(self, app):
        ...
        with app.app_context():
            doc = DocumentPresentationService.create(name="Temp Doc")
            doc_id = doc.id
    ...
    def test_delete_removes_from_database(self, app):
        ...
        with app.app_context():
            doc1 = DocumentPresentationService.create(name="Doc 1")
            doc2 = DocumentPresentationService.create(name="Doc 2")
...
class TestDocumentPresentationServiceIntegration:
    ...
    def test_complete_crud_cycle(self, app):
        ...
        with app.app_context():
            # Create
            doc = DocumentPresentationService.create(name="CRUD Doc", description="For testing")
            doc_id = doc.id
            
            # Read
            retrieved = DocumentPresentationService.get_by_id(doc_id)
            assert retrieved.name == "CRUD Doc"
            
            # Update
            updated = DocumentPresentationService.update(doc_id, name="Updated Doc")
            assert updated.name == "Updated Doc"
            
            # Verify update
            verified = DocumentPresentationService.get_by_id(doc_id)
            assert verified.name == "Updated Doc"
    ...
    def test_multiple_documents_lifecycle(self, app):
        ...
        with app.app_context():
            docs = [
                DocumentPresentationService.create(name=f"Doc {i}", description=f"Description {i}")
                for i in range(1, 4)
            ]
            ...
            DocumentPresentationService.create(name="New Doc")
            assert len(DocumentPresentationService.get_all()) == 3
